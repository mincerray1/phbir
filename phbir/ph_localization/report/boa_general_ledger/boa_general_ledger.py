...
def get_result(filters, account_details):
    ...
    clear_no_remarks(result)
    if filters.get('include_details_as_remarks'):
        print("add sub")
        add_sub_remarks(result)

    return result

def clear_no_remarks(result):
    for row in result:
        if 'remarks' in row and row['remarks'] == 'No Remarks':
            row['remarks'] = ''

def add_sub_remarks(result):
    for row in result:
    
        if 'remarks' in row:
            sub_remarks = row['remarks']

            if 'party_type' in row and row['party_type']:
                sub_remarks = sub_remarks + (', ' if sub_remarks else '') + 'Party Type: {0}'.format(row['party_type'])
            if 'party' in row and row['party']:
                sub_remarks = sub_remarks + (', ' if sub_remarks else '') + 'Party: {0}'.format(row['party'])
            if 'project' in row and row['project']:
                sub_remarks = sub_remarks + (', ' if sub_remarks else '') + 'Project: {0}'.format(row['project'])
            if 'cost_center' in row and row['cost_center']:
                sub_remarks = sub_remarks + (', ' if sub_remarks else '') + 'Cost Center: {0}'.format(row['cost_center'])
            if 'against_voucher_type' in row and row['against_voucher_type']:
                sub_remarks = sub_remarks + (', ' if sub_remarks else '') + 'Against Voucher Type: {0}'.format(row['against_voucher_type'])
            if 'against_voucher' in row and row['against_voucher']:
                sub_remarks = sub_remarks + (', ' if sub_remarks else '') + 'Against Voucher: {0}'.format(row['against_voucher'])
            if 'bill_no' in row and row['bill_no']:
                sub_remarks = sub_remarks + (', ' if sub_remarks else '') + 'Bill: {0}'.format(row['bill_no'])
                
            row['remarks'] = sub_remarks
...
def get_columns(filters):
    ...
    # set boa general ledger minimum columns
    columns = [
        {
            "label": _("GL Entry"),
            "fieldname": "gl_entry",
